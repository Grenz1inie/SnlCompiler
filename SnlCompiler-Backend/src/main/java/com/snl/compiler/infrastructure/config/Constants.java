package com.snl.compiler.infrastructure.config;

import java.util.ArrayList;
import java.util.List;

import com.snl.compiler.domain.grammar.Rule;
...
public class Constants {
    ...
    /**
     * 【Type 1: 分隔符/运算符 (Separator/Operator)】
     * - 什么是：SNL 源码中的单字符或双字符符号，如 \",\", \";\", \"+\", \":=\", \"..\" 等。
     * - 为什么要有：用于切分词素并表达语法结构，如赋值、区间、数组下标和表达式运算。
     * - 如何识别：词法分析器在扫描到符号字符时进行匹配，复合符号需要优先匹配更长的串。
     * - 识别了有什么用：lexer 用它把 token 编成“类型 1 + 下标”，parser 再据此还原具体符号；真正的 LL(1) 终结符表在 terminal 中。
     */
    public static List<String> separator; // 分隔符列表

    /**
     * 【Type 2: 保留字 (Reserved Words)】
     * - 什么是：SNL 预定义的关键字，如 \"program\", \"if\", \"while\" 等。
     * - 为什么要有：它们是语法结构的固定标记，不能作为用户自定义标识符使用。
     * - 如何识别：先识别字母开头的字符串，再到 reservedWord 表中查找，命中则归为保留字。
     * - 识别了有什么用：帮助词法分析器区分“关键字”和“普通标识符”，让后续语法分析按 token 类别进入相应分支。
     */
    public static List<String> reservedWord; // 保留字列表

    /**
     * 【Type 3: 标识符 (Identifier)】
     * - 什么是：用户自定义的名称，用于表示变量、类型名、过程名等。
     * - 为什么要有：允许程序员为程序实体命名，以便在不同作用域内引用数据或调用代码段。
     * - 如何识别：首字符必须匹配 L（字母表），后续字符可匹配 L 或 D（数字表）。
     * - 识别了有什么用：作为查找符号表（Symbol Table）的 Key，用于后续语义分析阶段的类型检查和地址分配。
     */
    public static List<Character> L; // 大小写字母表

    /**
     * 【Type 4: 整数常量 (Integer Constant)】
     * - 什么是：源代码中直接出现的十进制数字序列。
     * - 为什么要有：提供具体的即时数（Immediate），用于算术运算、循环计数或数组下标。
     * - 如何识别：通过 D（0-9）识别数字流，并在内部检查是否符合 SNL 整数规范（如非 0 开头）。
     * - 识别了有什么用：在词法阶段归一为 INTC，供后续语义、中间代码和目标代码阶段按常量值处理。
     */
    public static List<Character> D; // 0-9

    /**
     * 【Type 5: 整数常量首位字符集合 (Leading Digit Set for Integer Constant)】
     * - 什么是：十进制整数在词法分析中允许作为首字符的数字集合，通常是 1-9；0 只能单独成常量。
     * - 为什么要有：用来约束整数常量格式，避免非法的前导 0 写法。
     * - 如何识别：读取数字串时先检查首字符是否在 D1 中，或是否是单独的 0。
     * - 识别了有什么用：帮助 Lexer.isINTC() 快速判定整数常量是否合法。
     */
    public static List<Character> D1; // 1-9

    public static int[][] analysis = new int[68][38]; // LL(1)分析表

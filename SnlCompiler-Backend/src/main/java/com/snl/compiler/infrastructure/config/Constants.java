...
public class Constants {
     ...
     * - 什么是：SNL程序中用于断句、界定结构或执行运算的特殊符号，如 \",\", \";\", \"+\", \":=\" 等。
     * - 为什么要有：用于界定词素（Lexeme）的边界，表达程序逻辑（如赋值、算术、数组访问）。
     * - 如何识别：词法分析器扫描到非字母数字字符时进行匹配，支持贪婪匹配复合符号（如识别 \":=\" 而非 \":\"）。
     * - 识别了有什么用：通过 terminal 列表建立下标映射，直接驱动 LL(1) 分析法中分析栈的推导。
     */
    public static List<String> separator; // 分隔符列表

    /**
     * 【Type 2: 保留字 (Reserved Words)】
     * - 什么是：由系统预定义的具有固定语法语义的单词，如 \"program\", \"if\", \"while\" 等。
     * - 为什么要有：作为文法结构的“路标”，定义程序的基本控制流和声明逻辑，禁止被用户用作变量名以消除歧义。
     * - 如何识别：在识别出字母开头的字符串后，通过查表法（在 reservedWord 列表中检索）与普通标识符区分。
     * - 识别了有什么用：告知语法分析器当前语句的类型（是分支、循环还是定义），决定调用哪个递归下降函数。
     */
    public static List<String> reservedWord; // 保留字列表

    /**
     * 【Type 3: 标识符 (Identifier)】
     * - 什么是：用户自定义的名称，用于表示变量、过程、函数或新定义的类型。
     ...
     * - 识别了有什么用：在中间代码中解析为常量值，供后端生成汇编指令（如 MIPS 的 li 指令）。
     */
    public static List<Character> D; // 0-9

    /**
     * 【Type 5: 字符常量 (Character Constant)】
     * - 什么是：被单引号包围的单个字符，如 \'a\'。
     * - 为什么要有：支持程序处理字符数据类型。
     * - 如何识别：扫到单引号后读取中间内容，验证后续是否有闭合单引号，并检查长度。
     * - 识别了有什么用：在后端处理中会被转换为对应的 ASCII 数值进行存储和运算。
     */
